...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // Respond to the frontend immediately so it doesn't hang!
    res.json({
    ...
    // Send transactional email via Nodemailer (Gmail SMTP) IN THE BACKGROUND
    if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
      // Don't use await here, let it run in the background
      (async () => {
        try {
          ...
          console.error('❌ Failed to send email in background:', emailErr.message);
        }
      })();
    } else {
